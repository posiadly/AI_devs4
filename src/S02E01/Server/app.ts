...
const goodsUrl = requireEnv("S02E01_GOODS_URL");
...
app.post("/mcp", async (req, res) => {
  ...
  try {
    ...
    if (sessionId && transports.has(sessionId)) {
      ...
      const server = createServer(verificationUrl, apiKey, goodsUrl);
...
app.listen(3000, () => {
  console.log(
    "MCP Streamable HTTP Server running at http://localhost:3000/mcp",
  );
});
